// Package authcache authenticates alerting API keys. It verifies the key against
// the control plane with the "admin" scope and caches the result in Redis, so the
// common case is a single Redis GET. It mirrors the ingest and query authenticators.
package authcache
...
func (a *Authenticator) cacheKey(apiKey string) string {
	...
	return "auth:akey:" + hex.EncodeToString(sum[:])
}
